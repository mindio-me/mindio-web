    ...
    async importOne (node) {
      ...
      try {
        ...
        if (res && res.noteId) {
          ...
          this.$router.push({ path: '/workspace/notes', query: { openNoteId: res.noteId } })
        } else {
    ...
    async batchImport () {
      ...
      for (const node of nodes) {
        try {
          ...
          if (res && res.noteId) lastNoteId = res.noteId
          successCount++
        } catch (e) {
          failCount++
        }
      ...
      if (lastNoteId) {
        this.$router.push({ path: '/workspace/notes', query: { openNoteId: lastNoteId } })
      }
    },
    // ========== Wiki 节点类型辅助 ==========
    formatNodeTime (ts) {
