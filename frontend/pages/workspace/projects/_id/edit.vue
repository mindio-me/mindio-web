...
<template>
  <div class="project-editor-page">
    ...
    <el-dialog
      ...
      :append-to-body="true"
    >
      <UploadPanel :model="'note'" :pid="1" @success="handleUploadSuccess" @insert-html="handleInsertHtml" />
...
export default {
  ...
  methods: {
    ...
    handleInsertHtml(htmlContent) {
      this.uploadDialogVisible = false
      if (!htmlContent) return

      if (this.projectForm.contentType === 'richtext' && this.editor) {
        try {
          this.editor.cmd.do('insertHTML', htmlContent)
          this.projectForm.content = this.editor.txt.html()
        } catch (e) {
          this.projectForm.content = (this.projectForm.content || '') + htmlContent
        }
        this.$message.success('HTML 内容已插入正文')
      } else {
        this.$message.warning('仅富文本模式支持直接插入 HTML 文件内容')
      }
    },
    showTagInput() {
