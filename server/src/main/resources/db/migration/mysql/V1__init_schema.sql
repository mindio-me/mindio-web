...
--
-- 已登记的、刻意为之的 vendor 结构差异（见 MigrationVendorParityTest 注释）：
-- local_doc_directories / local_media_directories 的 (owner_id, dir_path) 唯一约束。
-- dir_path 是 varchar(1000)，utf8mb4 下 4000 字节，加 owner_id 共 4008 字节，超过 InnoDB
-- 索引键 3072 字节上限，真实 MySQL（5.7/8.0）建这个复合唯一索引会报 1071。MariaDB 会
-- 偷偷降级成 USING HASH 长唯一索引所以在 dev 上没暴露。这里改成：加一个 STORED 生成列
-- dir_path_hash = SHA2(dir_path, 256)（char(64)，ascii），唯一约束建在 (owner_id,
-- dir_path_hash) 上（8 + 64 = 72 字节），语义等价、任何引擎都能建。H2 没有这个长度限制，
-- h2/V1 保持原样的整列唯一（且 desktop 存量库已记录其 checksum，不能改），因此这两个表的
-- 结构在 h2/ 与 mysql/ 之间存在差异——语义一致：每个 (owner, path) 最多一条目录记录。
    ...
    create table local_doc_directories (
        ...
        dir_path_hash char(64) character set ascii generated always as (sha2(dir_path, 256)) stored not null,
    ...
    alter table local_doc_directories
       add constraint UKg0bmgl8xct5r0s6ykt5lcvs00 unique (owner_id, dir_path_hash);
    ...
    alter table local_media_directories
       add constraint UK7k4ybh60o95yf2t72pml9dc5j unique (owner_id, dir_path_hash);
