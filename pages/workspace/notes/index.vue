      ...
      <div class="tag-manager">
        ...
        <div class="tag-manager-body">
          <div class="tag-manager-list">
            <button
              v-for="tag in filteredManagedTags"
              :key="tag.id"
              type="button"
              class="tag-manager-row"
              :class="{ 'tag-manager-row--active': managedTagId === tag.id }"
              @click="selectManagedTag(tag)"
            >
              <span>{{ tag.name }}</span>
              <i class="el-icon-arrow-right"></i>
            </button>
              ...
              <el-form label-position="top">
                <el-form-item label="标签名称">
                  <el-input
                    ...
                    maxlength="50"
                    show-word-limit
                    @keyup.enter.native="saveManagedTag"
                  />
                </el-form-item>
              </el-form>
              <div class="tag-manager-editor-actions">
                <el-button
                  type="danger"
                  plain
                  size="small"
...
export default {
  ...
  mounted() {
    const openNoteId = this.$route.query.openNoteId
    if (openNoteId) {
      this.activeNoteId = Number(openNoteId)
      this.loadActiveNote()
...
<body>
  ...
  <div class="pdf-content">${contentHtml}</div>
    ...
    openTagManager() {
      ...
      if (this.tags.length > 0 && !this.managedTagId) {
        this.selectManagedTag(this.tags[0])
      }
    },
    selectManagedTag(tag) {
      if (!tag) return
      this.managedTagId = tag.id
      this.tagManagerForm = { name: tag.name || '' }
    },
    syncTagReferences(updatedTag) {
      const syncList = (list) => {
        if (!Array.isArray(list)) return
        list.forEach((tag) => {
          if (tag.id === updatedTag.id) tag.name = updatedTag.name
        })
      }
      syncList(this.activeNote?.tags)
      this.notes.forEach(note => syncList(note.tags))
      this.recentNotes.forEach(note => syncList(note.tags))
    },
    removeTagReferences(tagId) {
      this.selectedTags = this.selectedTags.filter(id => id !== tagId)
      const removeFromList = (list) => Array.isArray(list) ? list.filter(tag => tag.id !== tagId) : list
      if (this.activeNote) this.activeNote.tags = removeFromList(this.activeNote.tags)
      this.notes.forEach((note) => { note.tags = removeFromList(note.tags) })
      this.recentNotes.forEach((note) => { note.tags = removeFromList(note.tags) })
    },
    async createManagedTag() {
      const tagName = this.newTagName?.trim()
      if (!tagName) return
      ...
      try {
        const { data } = await this.$axios.post('/v1/tags', { name: tagName })
        this.tags.push(data)
        this.newTagName = ''
    ...
    async saveManagedTag() {
      const tagName = this.tagManagerForm.name?.trim()
      if (!this.managedTagId || !tagName) return

      this.tagManagerSaving = true
      try {
        await this.$axios.put(`/v1/tags/${this.managedTagId}`, { name: tagName })
    ...
    deleteManagedTag() {
      ...
      this.$confirm(`确定要删除标签 "${tag.name}" 吗？`, '提示', {
        ...
        this.tagManagerSaving = true
        try {
          await this.$axios.delete(`/v1/tags/${tag.id}`)
          this.tags = this.tags.filter(t => t.id !== tag.id)
          this.removeTagReferences(tag.id)
          const nextTag = this.filteredManagedTags[0] || this.tags[0]
          if (nextTag) {
            this.selectManagedTag(nextTag)
          } else {
