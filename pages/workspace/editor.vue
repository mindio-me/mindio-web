...
<template>
  <div class="editor-page" :class="{ 'theme-dark': isDarkTheme }">
        ...
        <div class="editor-content">
          ...
          <div class="doc-title-area">
            <input
              ...
              @input="onTitleChange"
              @keydown="handleTitleNavigationKeydown"
            />
            <div class="doc-meta">
    ...
    <transition name="lightbox-fade">
      <div v-if="lightbox.show" class="lightbox" @click.self="closeLightbox" @keydown.esc="closeLightbox">
  ...
  methods: {
    ...
    setupImagePaste(uploadService, noteId) {
      ...
      const handler = async (e) => {
        ...
        // 阻止 EditorJS 处理这次 paste（避免 paragraph invalid data 错误）
        e.stopPropagation()
        e.preventDefault()
      ...
      // useCapture=true：在 EditorJS 的 bubble 阶段 handler 之前执行
      editorEl.addEventListener('paste', handler, true)
      this._imagePasteHandler = () => editorEl.removeEventListener('paste', handler, true)
    },
    ...
    async onFileDrop(e) {
      ...
      let index = this.editor.blocks.getCurrentBlockIndex()
      if (index < 0) index = this.editor.blocks.getBlocksCount()

      for (const file of files) {
        await this.insertUploadedBlock(file, index)
        index++
      }
    },
    ...
    handleEditorImageClick(e) {
      ...
      if (e.target.closest('.ce-toolbar') || e.target.closest('.ce-toolbox')) return
      const allImgs = Array.from(document.querySelectorAll('#editorjs .image-tool img'))
      const images = allImgs.map(i => i.src).filter(Boolean)
      if (!images.length) return
      const index = allImgs.indexOf(e.target)
      this.lightbox = { show: true, images, index: index >= 0 ? index : 0 }
    },
    ...
    async insertUploadedBlock(file, index) {
      const mime = file.type || ''

      try {
        const result = await this.$uploadService.uploadLocal(file, 'note', this.currentNoteId || 0)
        const url = result.url || result.fileUrl || result
        ...
        if (mime.startsWith('image/')) {
          ...
          this.editor.blocks.insert('video',
            { url, caption: '', widthPercent: 100 },
            {}, index, true)
        } else if (mime.startsWith('audio/')) {
          this.editor.blocks.insert('audio',
            { url, caption: '' },
            {}, index, true)
        } else {
          this.editor.blocks.insert('paragraph',
            { text: `<a href="${url}">${file.name}</a>` },
            {}, index, true)
        }
