...
<template>
  <div class="local-media-page">
      ...
      <main class="workspace-main">
        ...
        <template v-else>
          ...
          <div class="media-toolbar">
            ...
            <el-switch
              v-if="$license && $license.isDesktop"
              v-model="videoAutoplayEnabled"
              class="video-autoplay-switch"
              active-text="静音预览"
              @change="refreshVideoPreviewObserver"
            />
          </div>
                ...
                <div class="media-waterfall">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                      <video
                        v-else-if="canShowVideoPreview(file)"
                        ref="mediaPreviewVideos"
            ...
            <transition name="panel-slide">
              <div v-if="selectedFile" class="detail-panel">
                ...
                <div v-else-if="selectedFile.mediaType === 'VIDEO'" class="panel-preview panel-preview--video">
                  <video :key="selectedFile.id" controls :src="fileUrl(selectedFile)" :data-file-id="selectedFile.id" @loadedmetadata="onVideoMetaLoaded"></video>
                </div>
    ...
    <!-- 添加目录 Dialog -->
    <el-dialog
      :title="$t('workspace.localMedia.dialogTitle')"
      :visible.sync="addDialogVisible"
      custom-class="local-media-directory-dialog"
      top="6vh"
      @close="resetForm"
    >
      <el-form ref="addForm" :model="addForm" :rules="addRules" label-width="80px">
        <el-form-item :label="$t('workspace.localMedia.dirPath')" prop="dirPath">
          <div class="path-input-row">
            ...
            <el-button icon="el-icon-folder-opened" style="margin-left:8px;flex-shrink:0" @click="chooseDirectory">{{ $t('workspace.localMedia.browse') }}</el-button>
          </div>
        </el-form-item>
  ...
  computed: {
    ...
    canPreviewVideos() {
      return process.client &&
        this.videoAutoplayEnabled &&
        !this.prefersReducedMotion &&
        Boolean(this.$license && this.$license.isDesktop)
    },
  },
  async created() {
  ...
  mounted() {
    ...
    if (process.client && window.matchMedia) {
      ...
      const updateMotionPreference = event => {
        ...
        this.refreshVideoPreviewObserver()
      }
      ...
      this.motionPreferenceQuery = motionQuery
    }
    this.refreshVideoPreviewObserver()
  },
  beforeDestroy() {
    clearTimeout(this.searchTimer)
    this.teardownVideoPreviewObserver()
    if (this.motionPreferenceQuery && this.motionPreferenceListener) {
      if (this.motionPreferenceQuery.removeEventListener) {
        this.motionPreferenceQuery.removeEventListener('change', this.motionPreferenceListener)
      } else if (this.motionPreferenceQuery.removeListener) {
        this.motionPreferenceQuery.removeListener(this.motionPreferenceListener)
      }
    }
  },
  methods: {
    ...
    selectFile(file) {
      ...
      if (this.selectedFile && this.selectedFile.mediaType === 'VIDEO') {
        this.pauseAllPreviewVideos()
      } else {
        this.playBestVisibleVideo()
      }
    },
    ...
    canShowVideoPreview(file) {
      if (!this.canPreviewVideos || file.mediaType !== 'VIDEO') return false
      const supportedExtensions = ['mp4', 'm4v', 'webm', 'mov']
      return supportedExtensions.includes((file.fileExtension || '').toLowerCase())
    },

    refreshVideoPreviewObserver() {
      if (!process.client) return
      this.$nextTick(() => {
        this.teardownVideoPreviewObserver()
        if (!this.canPreviewVideos || !window.IntersectionObserver) return

        const root = this.$refs.mediaScroller
        const videos = this.previewVideoElements()
        if (!root || videos.length === 0) return

        this.visiblePreviewVideos = new Map()
        this.videoPreviewObserver = new IntersectionObserver(this.handleVideoPreviewIntersections, {
          root,
          threshold: [0, 0.35, 0.6, 0.85],
        })
        videos.forEach(video => this.videoPreviewObserver.observe(video))
      })
    },

    teardownVideoPreviewObserver() {
      if (this.videoPreviewObserver) {
        this.videoPreviewObserver.disconnect()
        this.videoPreviewObserver = null
      }
      if (this.videoPreviewRaf) {
        cancelAnimationFrame(this.videoPreviewRaf)
        this.videoPreviewRaf = null
      }
      this.visiblePreviewVideos = new Map()
      this.pauseAllPreviewVideos()
    },

    previewVideoElements() {
      const videos = this.$refs.mediaPreviewVideos || []
      return Array.isArray(videos) ? videos : [videos]
    },

    handleVideoPreviewIntersections(entries) {
      if (!this.visiblePreviewVideos) this.visiblePreviewVideos = new Map()
      entries.forEach(entry => {
        const id = entry.target.dataset.fileId
        if (!id) return
        if (entry.isIntersecting && entry.intersectionRatio >= 0.6) {
          this.visiblePreviewVideos.set(id, {
            el: entry.target,
            ratio: entry.intersectionRatio,
          })
        } else {
          this.visiblePreviewVideos.delete(id)
          if (entry.target.dataset.previewPlaying === 'true') {
            entry.target.pause()
            entry.target.dataset.previewPlaying = 'false'
          }
        }
      })
      this.queueVideoPreviewPlayback()
    },

    queueVideoPreviewPlayback() {
      if (this.videoPreviewRaf) return
      this.videoPreviewRaf = requestAnimationFrame(() => {
        this.videoPreviewRaf = null
        this.playBestVisibleVideo()
      })
    },

    playBestVisibleVideo() {
      if (!this.canPreviewVideos || !this.visiblePreviewVideos || this.visiblePreviewVideos.size === 0) {
        this.pauseAllPreviewVideos()
        return
      }
      if (this.selectedFile && this.selectedFile.mediaType === 'VIDEO') {
      ...
      const candidates = Array.from(this.visiblePreviewVideos.entries())
        .filter(([, item]) => item.el.dataset.previewError !== 'true')
        .sort(([, a], [, b]) => {
          const aRect = a.el.getBoundingClientRect()
          const bRect = b.el.getBoundingClientRect()
          const aDistance = Math.abs((aRect.top + aRect.height / 2) - rootCenter)
          const bDistance = Math.abs((bRect.top + bRect.height / 2) - rootCenter)
          return aDistance - bDistance
        })

      const nextId = candidates.length ? candidates[0][0] : null
      this.previewVideoElements().forEach(video => {
        const shouldPlay = nextId && video.dataset.fileId === nextId
        if (!shouldPlay) {
          video.pause()
          video.dataset.previewPlaying = 'false'
          return
        }
        video.muted = true
        const playPromise = video.play()
        video.dataset.previewPlaying = 'true'
        this.playingPreviewVideoId = nextId
    ...
    pauseAllPreviewVideos() {
      ...
      this.playingPreviewVideoId = null
    },

    onVideoMetaLoaded(event) {
      const fileId = event.target.dataset.fileId
      const w = event.target.videoWidth
      const h = event.target.videoHeight
      if (fileId && w && h) {
        this.$set(this.videoDimensions, fileId, { width: w, height: h })
      }
    },

    markVideoPreviewError(event) {
      event.target.dataset.previewError = 'true'
      if (event.target.dataset.previewPlaying === 'true') {
        event.target.pause()
        event.target.dataset.previewPlaying = 'false'
      }
      if (this.visiblePreviewVideos) {
        this.visiblePreviewVideos.delete(event.target.dataset.fileId)
      }
      this.playBestVisibleVideo()
    },
    ...
    fileAspectRatio(file) {
      if (file.mediaType === 'IMAGE' && file.imageWidth && file.imageHeight) {
        return file.imageWidth / file.imageHeight
      }
      if (file.mediaType === 'VIDEO') {
        const dims = this.videoDimensions[file.id]
        if (dims && dims.width && dims.height) return dims.width / dims.height
        return 16 / 9
      }
      if (file.mediaType === 'AUDIO') return 1
      return 4 / 3
    },

    cardFlexStyle(file) {
      return { flexBasis: `${Math.round(180 * this.fileAspectRatio(file))}px` }
    },

    thumbAspectStyle(file) {
      return { aspectRatio: String(this.fileAspectRatio(file)) }
    },
...
:deep(.local-media-directory-dialog) {
  width: 70vw;
  max-width: 1040px;
  min-width: 680px;
