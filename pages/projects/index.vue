...
<template>
  <div class="projects-page">
    ...
    <!-- Achievements — Backend-driven Showcase -->
    <section v-if="achievements.length" class="own-products-section">
      <div class="own-products-container">
        ...
        <div class="own-products-grid">
          <div
            ...
            :key="achievement.id"
            class="own-product-card"
          >
            <div class="op-left">
              <div class="op-icon" :class="'op-icon--' + (achievement.iconVariant || 'purple')">
                <i :class="achievement.icon || 'el-icon-trophy'"></i>
              </div>
            </div>
            <div class="op-body">
              <div class="op-meta">
                <span v-if="achievement.type" class="op-type">{{ achievement.type }}</span>
                <span class="op-status" :class="'op-status--' + (achievement.status || 'active')">
                  {{ getAchievementStatusLabel(achievement.status) }}
                </span>
              </div>
              <h3 class="op-title">{{ achievement.title }}</h3>
              <p v-if="achievement.subtitle" class="op-sub">{{ achievement.subtitle }}</p>
              <div class="op-desc wangeditor-content" v-html="achievement.description"></div>
              <div v-if="achievement.technologies" class="op-tech">
                <span
                  v-for="tech in achievement.technologies.split(',').map(t => t.trim()).filter(Boolean)"
                  :key="tech"
                  class="tag"
                >
                  {{ tech }}
...
export default {
  ...
  async asyncData({ app }) {
    const [projects, achievements] = await Promise.all([
      app.$projectService.getPublicProjects().catch(() => []),
      app.$achievementService.getPublicAchievements().catch(() => [])
    ])
  ...
  methods: {
    ...
    getAchievementStatusLabel(status) {
      const map = {
        active: this.$t('projectsPage.ownProducts.statusActive'),
        done: this.$t('projectsPage.ownProducts.statusDone')
      }
...
.op-desc {
  ...
  // 覆盖 .wangeditor-content 默认的文章排版尺寸，保持卡片内紧凑排版
  &.wangeditor-content {
    padding: 0;
